...
namespace QEyeCommon.Tool
{
	...
	public class Log
	{
		 ...
		 * 
		 */
		private ListDictionary m_Logs;
		private LogMethods m_LogMethods;
		private elogprofile m_logprofile;
		private bool m_DefaultToPrimaryProfile = true;
        ...
        public Log(eloglevel logLevel)
            :this(DefaultLogFileAndLocation()[1],DefaultLogFileAndLocation()[0],logLevel)
		{
		}
		public Log(string logFileName, string logLocation, eloglevel logLevel)
		{
			...
			*/
            this.Init();
			this.Profile = elogprofile.primary;
			//I prefer a simple text file log but you can change this 
			//to use the event log also.
            this.LogWhere = elogwhere.file;

            if (Convert.ToInt32(this.LogLevel) < 1)
			{
                this.LogLevel = eloglevel.error; //errors show at the minimum.
			}
			else
			{
		...
		public void LogThis(elogprofile logprofile, string logtext,eloglevel loglevel)
		{
			((LogMethods)m_Logs[Convert.ToString(logprofile)]).LogThis(logtext,loglevel);
		}
		public void LogThis(elogprofile logprofile, string logtext,eloglevel loglevel, elogprefix logprefix)
		{
		...
		public bool DefaultToPrimaryProfile
		{
			get
			{
		...
		public void SetLogPath(elogprofile logprofile)
		{
		...
		public void LogReset(elogprofile logprofile)
		{
			((LogMethods)m_Logs[Convert.ToString(logprofile)]).LogReset();
		}
		public void LogHeader(string sText, elogheaderlevel logheaderlevel)
		{
			if (m_DefaultToPrimaryProfile)
			{
		...
		public void LogHeader(elogprofile logprofile, string sText,elogheaderlevel logheaderlevel)
		{
			string sHeader = "";
		...
		public LogMethods GetProfile(elogprofile logprofile)
		{
	...
	public class LogMethods //: System.IDisposable
	{
		...
		private void AppendToFile(string sPath, string sText)
		{
            try
            {
                ...
                {
                    String path = sPath.Substring(0, sPath.LastIndexOf("\\".ToCharArray()[0]));
